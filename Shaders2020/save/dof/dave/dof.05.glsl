...
float rand(vec2 coords, float seed) {
    return fract(sin(dot(coords.xy, vec2(12.989 + mod(seed, 10.0), 78.233))) * 43758.5453);
}
...
void main(void) {
    ...
    vec4 one = texture2D(adsk_results_pass4, st);

    if (outcol == 0) {
        float blur_factor = texture2D(BLURFACTOR, st).a;
        vec4 blurred_out = vec4(0.0);
        float matte = texture2D(HIGHLIGHTCC, st).a;
        ...
        for(float y = 0.0; y <= 1.0; y += 1.0 / s) {
            for(float x = 0.0; x <= 1.0; x += 1.0 / s) {
                ...
                if (outcol != 3) {
                    ...
                    blur = texture2D(adsk_results_pass1, sample_coords);
        ...
        */
        ...
        float alpha = clamp(blurred_out.a + one.a, 0.0, 1.0);
        blurred_out = mix(one, blurred_out, alpha);
        blurred_out.a = alpha;
