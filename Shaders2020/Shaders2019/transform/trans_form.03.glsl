...
#define Warped adsk_results_pass2
...
void main(void) {
  ...
  //output warp only
  vec3 outcol = warped.rgb;

  if (output_comp == 1) {
    //comp over front
    warped.rgb /= max(warped.a, .000001);
    outcol = mix(front, warped.rgb, warped.a);
  } else if (output_comp == 2) {
    //comp over missing front with peice missing
    warped.rgb /= max(warped.a, .000001);

    float matte = texture2D(Matte,st).r;
    front *= clamp((1.0 - matte) + warped.a, 0.0, 1.0);
    outcol = mix(front, warped.rgb, warped.a);
  }

	outcol = do_colorspace(outcol, 1);

	gl_FragColor = vec4(outcol, warped.a);
}
