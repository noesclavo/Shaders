...
#define HorizontalBlurred adsk_results_pass4
uniform sampler2D HorizontalBlurred;
...
vec4 blur()
{
	...
	vec4 blurred = g0 * texture2D(HorizontalBlurred, st);
	vec4 pass = g0;
	g0 *= g1;
	g1 *= g2;

	for(float i = 1; i <= s * 3.0; i++) {
		st = (uv - i * dir) * texel;
		st = repeat_coords(st);

    blurred += g0 * texture2D(HorizontalBlurred, st);
