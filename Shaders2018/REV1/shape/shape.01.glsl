...
float draw_shape(vec2 point, vec2 top, vec2 left, vec2 right)
{
	...
	if (lr == vmin &&  uvb.x > 0.0)
	{
		...
		if (theta_lpr > s)
		{
			float edge = 1.0 - aa2 / max(theta_lpr, .00001);
	...
	else if (uvl.y > 0.0 && lr == vmin && lt == vmin)
	{
		...
		if (theta_tpl > s)
		{
			float edge = 1.0 - aa2 / max(theta_tpl, .000001);
	...
	else if (uvr.y > 0.0 && point.x > top.x && lr == vmin && rt == vmin)
	{
		...
		if (theta_tpr > s)
		{
			float edge = 1.0 - aa2 / max(theta_tpr, .00001);
...
void main(void) {
	...
	float shape = 0.0;
 	...
 	vec3 f_offset = getRGB( fg.x / 360.0, fg.y * .01 ) * vec3( fg.z * 0.01);
  vec3 fgcol = f_offset;
