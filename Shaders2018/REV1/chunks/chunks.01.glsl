...
void main(void) {
	...
	vec4 orig = texture2D(Front, st);
	float strength = texture2D(Strength, st).r;

	vec2 rect_size = vec2(size_x, size_y);
	...
	vec2 s = type == 0 ? vec2(square_size + rmult) * texel : (rect_size + vec2(rmult)) * texel;
	...
	vec2 spread = vec2(spread_x, spread_y) * vec2(uniform_spread_size) * vec2(strength);
