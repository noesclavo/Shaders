...
void main(void) {
	...
	vec2 pos = vec2(pos_x, pos_y) * texel;
	vec2 scale = vec2(100) / vec2(scale_x, scale_y);
	...
	if (op == 0) {
		...
		vec2 uv_scale = mix(scale, vec2(1.0), strength * scale);
		uv /= uv_scale;
	}
	...
	gl_FragColor = vec4(front.rgb, influence);
	//gl_FragColor = front;
}
