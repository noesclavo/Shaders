...
#define Front_RGBA adsk_results_pass1
#define Strength adsk_results_pass3
#define DirBlur adsk_results_pass4
#define GBlur adsk_results_pass5

uniform float adsk_result_w, adsk_result_h;
vec2 res = vec2(adsk_result_w, adsk_result_h);

uniform sampler2D Front;
uniform sampler2D Front_RGBA;
uniform sampler2D Strength;
uniform sampler2D DirBlur;
...
void main(void) {
  ...
  float matte = texture2D(Front_RGBA, st).a;
  vec4 blurred = vec4(0.0);

  if (blur_type == 0) {
	   blurred = texture2D(GBlur, st);
  } else if (blur_type == 1) {
	   blurred = texture2D(DirBlur, st);
  }
  ...
  if (show_vectors && strength_are_vectors) {
    comp = texture2D(Strength, st).rgb;
  } else {
