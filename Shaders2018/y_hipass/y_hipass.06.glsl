...
void main(void) {
  ...
  vec3 blur = texture2D(adsk_results_pass3, st).rgb;
  vec3 color = texture2D(adsk_results_pass5, st).rgb;
  ...
  if (recombine) {
    vec3 nhi = texture2D(hi_pass, st).rgb;

    nhi = mix(nhi, comp, blend);
    nhi -= m;
    nhi /= vec3(gain);
    comp = vec3(color + low + nhi);
