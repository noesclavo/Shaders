...
void main(void) {
	...
	vec4 blurred = texture2D(INPUT1, st);
	blurred.rgb = do_colorspace(blurred.rgb, 1);

    vec3 comp = mix(front, blurred.rgb, matte);
    if (o == 1) {
        comp = mix(front, blurred.rgb, blurred.a);
    }


	gl_FragColor = vec4(comp, blurred.a);
}
