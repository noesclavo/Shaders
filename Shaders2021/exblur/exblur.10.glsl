...
uniform bool screen_over_front;
uniform float mixx;
...
void main(void) {
    ...
    vec4 one = clamp(texture2D(adsk_results_pass3, st), 0.0, 5.0);
    vec4 two = clamp(texture2D(adsk_results_pass5, st), 0.0, 5.0);
    vec4 three = clamp(texture2D(adsk_results_pass7, st), 0.0, 5.0);
    vec4 four = clamp(texture2D(adsk_results_pass9, st), 0.0, 5.0);

    vec4 outcol = one;

    if (blur_amount >= 1.0) {
        outcol = one + two + three + four;
    }

    outcol.a = clamp(outcol.a, 0.0, 1.0);

    outcol.rgb = mix(front.rgb, outcol.rgb, outcol.a);
    if (screen_over_front) {
        vec4 screen = adsk_getBlendedValue(17, front, outcol);
        outcol.rgb = mix(outcol.rgb, screen.rgb, mixx * .01);
    }
