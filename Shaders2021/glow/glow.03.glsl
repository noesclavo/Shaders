...
void main(void) {
    ...
    vec4 front_premult = texture2D(adsk_results_pass1, st);
    ...
    if (blend_mode == -1) {
        ...
        outcol = front_premult;
        alpha = matte;
    } else {
        outcol = adsk_getBlendedValue(blend_mode, blurred * vec4(strength), front);
        alpha = blurred.a * strength;
    }

    outcol.rgb = do_colorspace(outcol.rgb, 1);

    gl_FragColor = vec4(outcol.rgb, alpha);
}
