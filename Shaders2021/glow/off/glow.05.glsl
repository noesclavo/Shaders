...
uniform sampler2D adsk_results_pass4;   // Premultiplied Input
uniform sampler2D adsk_results_pass5;   // Output of horizontal blur execution pass (the first pass);;             
...
void main(void) {
    ...
    if (! exponential_blur) {
        ...
        gl_FragColor = blur(adsk_results_pass4, eblur, e);
    } else {
        gl_FragColor = blur(adsk_results_pass5, eblur, e);
    }
}
