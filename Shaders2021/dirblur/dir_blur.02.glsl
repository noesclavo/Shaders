...
#define PI 3.14159
#define FRONTLINIEAR adsk_results_pass1
...
uniform sampler2D FRONTLINIEAR;
...
vec4 gblur()
{
    ...
    vec2 strength = vec2(0.0);

    // Optional texture used to weight amount of blur
    #ifdef STRENGTH_CHANNEL
        strength = abs(texture2D(Strength, uv * texel).rg);
    #endif
    ...
    vec4 blurred = g0 * texture2D(FRONTLINIEAR, st);
    float pass = g0;
    g0 *= g1;
    g1 *= g2;

    for(float i = 1; i <= blur_amount * 3.0; i++) {
        st = (uv - i * dir) * texel;
        st = repeat_coords(st);

        blurred += g0 * texture2D(FRONTLINIEAR, st);
        pass += g0;
