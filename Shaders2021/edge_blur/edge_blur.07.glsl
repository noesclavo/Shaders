...
void main(void) {
    ...
    vec4 orig = texture2D(adsk_results_pass1, st);
    vec3 blurred = texture2D(adsk_results_pass6, st).rgb;
    float edge = texture2D(adsk_results_pass4, st).r;

    blurred = do_colorspace(blurred, 1);
    ...
    gl_FragColor = vec4(blurred, alpha);
}
