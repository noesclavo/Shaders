...
vec2 repeat_coords(vec2 coords, vec4 uv_bounds, int repeat) {
    //tile
    if (repeat == 1) {
    ...
    //mirror
        ...
        if (coords.x < uv_bounds.x || coords.x > uv_bounds.z) {
            discard;
        }
        if (coords.y < uv_bounds.y || coords.y > uv_bounds.w) {
            discard;
        }
    }
...
void main(void) {
    ...
    */
    ...
    uv = repeat_coords(uv, uv_bounds, 3);
