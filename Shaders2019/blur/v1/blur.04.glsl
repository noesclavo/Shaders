...
#define PI 3.1415
#define Blurred adsk_results_pass3
...
void main(void) {
    ...
    if (keep_inside) {
        float matte = texture2D(Matte, st).a;

        blurred /= max(blurred.a, .000000001);
            outcol.rgb = mix(front, blurred.rgb, matte);
    } else {
        outcol.rgb = mix(front, outcol.rgb, outcol.a);
    }
